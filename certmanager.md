...
Initial Observation
	...
	•	oc describe certificate showed the error:
...
When reconciliation didn’t trigger:
...
oc logs -l app=aws-privateca-issuer -n cert-manager --tail=100
	...
	3.	If AWS auth fails → Verify SA annotation and restart pod.
	4.	oc project openshift-ingress
	5.	Recheck Certificate and delete failed requests:

oc get certificaterequest -n openshift-ingress
oc delete certificaterequest <name>


	6.	If Certificate is stuck → delete and recreate it from backup YAML.
